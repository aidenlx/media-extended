...
### Remote Media Files [#hosted-media-files]

Media Extended supports opening videos and audio from publicly hosted media files, commonly served via WebDAV, or static file servers.
...
For password protected files, you should go to settings, open platformsassword manager, and add the password to the specific domain and(or) path before opening the file.
